...
function App() {
  ...
  async function ask(value = question) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: 'assistant', text: data.answer, citations: data.citations || [], draft: data.draft_created }])
    } catch (error) {
  ...
  async function uploadKnowledge(file) {
    ...
    const canEditKnowledge = department.toLowerCase() === 'hr' || access === 'admin'
    if (!canEditKnowledge) {
      setUploadResult({ error: 'Chỉ HR hoặc admin được đưa dữ liệu mới vào Obsidian.' })
      return
    }
    setUploading(true)
    ...
    form.append('user_department', department || 'HR')
    form.append('user_access_level', access)
    try {
      const response = await fetch('/api/knowledge/upload', { method: 'POST', body: form })
      const data = await response.json()
      if (!response.ok) throw new Error(data.detail || `Backend trả về HTTP ${response.status}`)
      setAttachedFile(null)
      setUploadResult(data)
      setMessages((current) => [...current, { role: 'assistant', text: data.message, upload: data }])
      loadHealth()
    } catch (error) {
      setUploadResult({ error: error.message || 'Không thể upload dữ liệu.' })
    } finally {
      setUploading(false)
  ...
  async function debugSearch(event) {
    ...
    setDebugging(true)
    try {
      const response = await fetch(`/api/debug/search?q=${encodeURIComponent(debugQuery)}&user_access_level=${access}`)
        ...
        <main className="relative min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-12 lg:py-9 xl:px-16">
          <div className="mx-auto max-w-[1150px]">
            <div className="flex flex-col justify-between gap-5 border-b border-[#dce3de] pb-8 md:flex-row md:items-start">
              ...
              <div className="flex items-center gap-2 rounded-full border border-[#dce3de] bg-white/70 px-3 py-2 text-xs text-[#607175]"><span className={`status-dot ${statusOnline ? 'online' : ''}`} />{statusOnline ? `${health.vault_approved_files || 0}/${health.vault_markdown_files || 0} approved notes` : 'Backend offline'}</div>
            </div>
            ...
            <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]">
              <section className="paper-panel flex min-h-[650px] flex-col overflow-hidden rounded-2xl">
                <div className="flex items-center justify-between border-b border-[#e2e7e1] px-5 py-4 sm:px-7"><div><p className="text-sm font-bold text-ink">Staff conversation</p><p className="mt-1 text-[11px] text-[#889598]">Grounded assistant · {access} access</p></div><div className="flex items-center gap-2 rounded-full bg-[#e8f4ed] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.12em] text-[#24725f]"><Sparkles size={13} /> RAG ready</div></div>
                ...
                <form className="border-t border-[#e2e7e1] bg-[#fcfbf7] p-4 sm:p-5" onSubmit={(event) => { event.preventDefault(); attachedFile ? uploadKnowledge(attachedFile) : ask() }}><div className="relative flex items-center"><input value={question} onChange={(event) => setQuestion(event.target.value)} className="h-14 w-full rounded-xl border border-[#d8dfda] bg-white pl-4 pr-24 text-sm text-ink outline-none transition placeholder:text-[#9aa5a5] focus:border-[#3c8b79] focus:ring-4 focus:ring-[#3c8b79]/10" placeholder={attachedFile ? 'File đã chọn, bấm gửi để đưa vào Obsidian...' : 'Hỏi về quy định nội bộ...'} /><input ref={fileInputRef} type="file" className="hidden" accept=".md,.txt,.csv,.json,.pdf,.docx,.png,.jpg,.jpeg,.webp" onChange={selectFile} /><button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="absolute right-14 grid h-10 w-10 place-items-center rounded-lg text-[#287661] transition hover:bg-[#e8f4ed] disabled:opacity-40" aria-label="Đính kèm dữ liệu"><Paperclip size={18} /></button><button disabled={loading || uploading || (!question.trim() && !attachedFile)} className="absolute right-2 grid h-10 w-10 place-items-center rounded-lg bg-[#e9795c] text-white transition hover:bg-[#d9654a] disabled:cursor-not-allowed disabled:opacity-35" aria-label={attachedFile ? 'Upload vào Obsidian' : 'Gửi câu hỏi'}>{attachedFile ? <Check size={18} strokeWidth={2.5} /> : <ArrowUp size={18} strokeWidth={2.5} />}</button></div>{attachedFile && <div className="mt-2 flex items-center justify-between rounded-lg bg-[#e8f4ed] px-3 py-2 text-[11px] text-[#287661]"><span className="truncate"><Paperclip size={12} className="mr-1 inline" />{attachedFile.name}</span><button type="button" onClick={() => setAttachedFile(null)} className="ml-2 text-[#a55f48]">Bỏ file</button></div>}{uploadResult && <div className={`mt-2 text-[11px] ${uploadResult.error ? 'text-[#a55f48]' : 'text-[#287661]'}`}>{uploadResult.error || 'Đã lưu vào Obsidian dưới dạng draft.'}</div>}<div className="mt-3 flex items-center justify-between px-1 text-[10px] text-[#9aa5a5]"><span>Chỉ HR/admin được thêm dữ liệu · file tối đa 10MB</span><span>Vietnamese · Internal</span></div></form>
              </section>

              <aside className="space-y-5">
                <section className="paper-panel rounded-2xl p-5"><div className="flex items-start justify-between"><div><p className="panel-kicker">Your context</p><h2 className="panel-title mt-1">Access scope</h2></div><ShieldCheck className="text-[#3d8d79]" size={20} /></div><p className="mt-3 text-xs leading-5 text-[#7c898a]">Context được lọc trước khi đưa vào pipeline RAG.</p><Field label="Phòng ban"><Select value={department} onChange={(event) => setDepartment(event.target.value)}><option value="">Tất cả phòng ban</option><option>HR</option><option>Finance</option><option>Academic</option><option>Admin</option></Select></Field><Field label="Quyền người dùng"><Select value={access} onChange={(event) => setAccess(event.target.value)}><option value="staff">Staff</option><option value="manager">Manager</option><option value="admin">Admin</option></Select></Field><div className="mt-4 flex items-center gap-2 border-t border-[#e7ebe6] pt-4 text-[11px] text-[#718081]"><Check size={14} className="text-[#3d8d79]" /> Approved sources only</div></section>
                <section className="paper-panel rounded-2xl p-5"><div className="flex items-start justify-between"><div><p className="panel-kicker">Vault control</p><h2 className="panel-title mt-1">Knowledge sync</h2></div><Database className="text-[#3d8d79]" size={20} /></div><p className="mt-3 text-xs leading-5 text-[#7c898a]">Đồng bộ các note approved từ Obsidian Vault.</p><button onClick={reindex} disabled={reindexing} className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[#123d52] text-xs font-bold text-white transition hover:bg-[#1a5169] disabled:opacity-50"><RefreshCw size={14} className={reindexing ? 'animate-spin' : ''} /> {reindexing ? 'Indexing...' : 'Reindex Vault'}</button>{reindexResult && <div className="mt-3 rounded-lg bg-[#edf6ef] p-3 text-[11px] leading-5 text-[#36725e]">{reindexResult.error ? 'Không thể kết nối backend.' : <><strong>{reindexResult.indexed_files} files</strong> · {reindexResult.indexed_chunks} chunks · {reindexResult.graph_edges} links</>}</div>}</section>
...
function Message({ message }) { return <div className={`flex gap-3 ${message.role === 'user' ? 'justify-end' : ''}`}>{message.role !== 'user' && <div className="avatar"><Bot size={16} /></div>}<div className={`max-w-[86%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === 'user' ? 'user-bubble' : 'assistant-bubble'}`}>{message.text}{message.citations?.length > 0 && <div className="mt-3 border-t border-[#3d8d79]/20 pt-2 text-[10px] leading-5 text-[#29745f]"><div className="mb-1 flex items-center gap-1 font-bold uppercase tracking-[.08em]"><Link2 size={11} /> Sources</div>{message.citations.map((citation, index) => <div key={`${citation.source}-${index}`}>[{citation.source} · {citation.heading} · v{citation.version}]</div>)}</div>}{message.draft && <div className="mt-2 text-[10px] text-[#a55f48]">Đã tạo draft để HR bổ sung.</div>}</div>{message.role === 'user' && <div className="avatar user-avatar">You</div>}</div> }
